...
export default function LandingPage() {
  return (
    <div className="min-h-screen bg-[var(--bg-base)] text-[var(--text-primary)]">
      <nav className="fixed inset-x-0 top-0 z-50 flex h-14 items-center justify-between border-b border-[var(--border)] bg-[var(--bg-base)]/80 px-8 backdrop-blur">
      ...
      <section className="relative flex min-h-screen flex-col items-center justify-center px-4 pt-14 text-center">
        ...
        <div className="relative">
          <span className="mb-4 inline-flex items-center gap-1.5 rounded-full border border-[var(--accent-muted)] bg-[var(--accent-muted)] px-3 py-1 text-xs font-medium text-[var(--accent)]">
          ...
          <h1 className="mt-4 text-4xl font-bold leading-tight tracking-tight md:text-5xl lg:text-6xl">
            ...
            <span className="text-[var(--accent)]">en toute clarté</span>
          </h1>
      ...
      <section className="border-y border-[var(--border)] bg-[var(--bg-surface)] py-12">
      ...
      <section className="mx-auto max-w-5xl px-8 py-20">
        <div className="mb-12 text-center">
          ...
          <p className="mt-2 text-sm text-[var(--text-muted)]">Des modules conçus pour la gestion quotidienne de votre activité</p>
        </div>
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {features.map((f) => {
            const Icon = f.icon;
            return (
              <div key={f.title} className="rounded-xl border border-[var(--border)] bg-[var(--bg-card)] p-5 transition-colors hover:border-[var(--border)]">
                <div className="mb-3 flex h-9 w-9 items-center justify-center rounded-lg bg-[var(--accent-muted)]">
      ...
      <section className="py-20 text-center">
        <div className="mx-auto max-w-md px-8">
          <Globe size={32} className="mx-auto mb-4 text-[var(--accent)]" />
          <h2 className="text-2xl font-bold">Prêt à démarrer ?</h2>
          <p className="mt-2 text-sm text-[var(--text-muted)]">Connectez-vous et prenez le contrôle de votre plateforme e-commerce.</p>
          <div className="mt-6">
            <Link href="/login">
